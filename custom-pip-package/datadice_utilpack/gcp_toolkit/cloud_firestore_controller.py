...
class CloudFirestoreController:
    ...
    def retryable_operation(func, *args: Any, **kwargs: Any):
        """
        Retry a Firestore operation with exponential backoff.

        This static method retries a function (intended to be a Firestore operation) using a
        retry mechanism with exponential backoff and fixed wait intervals.

        Args:
            func (callable): The Firestore operation function to retry.
            *args (Any): Variable length argument list for the Firestore operation function.
            **kwargs (Any): Arbitrary keyword arguments for the Firestore operation function.

        Returns:
            The result of the Firestore operation function.
        """
        logger.debug(
    ...
    def modify_counter_by_path(
        ...
        Args:
            ...
            increment_by (int, optional): The amount to increment by. Defaults to 1.
    ...
    def query_collection_by_path(
        ...
        """
        Query a Firestore collection by path with optional query parameters, sorting, and pagination.

        Args:
            path (list): The path to the Firestore collection.
            query_params (list, optional): A list of query parameters for filtering documents.
            sort_by (tuple, optional): A tuple containing the field name to sort by and the sort direction.
            limit (int, optional): The maximum number of documents to return.
            start_after (document, optional): A document snapshot to start the query after.

        Returns:
            A list of dictionaries representing the queried documents.
        """
    ...
    def query_collection_by_path_pagination(
        ...
        """
        Query a Firestore collection by path with pagination, using provided query parameters and sorting.

        Args:
            path (list): The Firestore collection path.
            query_params (list, optional): Parameters to filter the collection documents.
            sort_by (tuple, optional): Field name and direction ('ASC' or 'DESC') for sorting results.
            limit (int, optional): Maximum number of documents to return.
            start_after (document, optional): Document snapshot to start the query (for pagination).

        Returns:
            A tuple containing a list of queried documents and the last document snapshot.
        """
        ref_type, collection_ref = self.get_reference_by_path(path)
    ...
    def delete_documents_from_collection_by_query(
        ...
        Args:
            path (list): The path to the Firestore collection.
            query_params (list, optional): Parameters to filter which documents will be deleted.
    ...
    def delete_batch(self, docs, query_params=None, sort_by=None):
        ...
        Args:
            docs (list): A list of Firestore document references to delete.
            query_params (dict, optional): Query parameters, unused in this method but retained for consistency.
            sort_by (any, optional): Sorting parameters, unused in this method but retained for interface consistency.
        ...
        batch.commit()
        logger.debug(
            f"Deleted {len(docs)} documents. Last item: {docs[-1].reference.path}. "
            f"Query params: {query_params}. Sort by: {sort_by}"
        )
        return len(docs)
    ...
    def delete_documents_from_collection_group_by_query(
        ...
        """
        Delete documents from a Firestore collection group based on query parameters.

        Args:
            collection_group (str): The collection group from which to delete documents.
            query_params (list): Query parameters to filter which documents will be deleted.
            batch_size (int): The number of documents to delete in each batch.
            sort_by (tuple): Field name and direction ('ASC' or 'DESC') for sorting documents before deletion.
            thread_multiplier (int): Multiplier for the number of threads to use based on the number of available CPUs.

        Notes:
            This method uses multithreading to speed up the deletion process across potentially large collection groups.
        """
